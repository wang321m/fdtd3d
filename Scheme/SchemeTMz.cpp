...
#ifdef COMPLEX_FIELD_VALUES
        tmp->setCurValue (FieldValue (sin (gridTimeStep * t * 2 * PhysicsConst::Pi * 30000000000),
                                      cos (gridTimeStep * t * 2 * PhysicsConst::Pi * 30000000000)));
#else /* COMPLEX_FIELD_VALUES */
        tmp->setCurValue (sin (gridTimeStep * t * 2 * PhysicsConst::Pi * 30000000000));
#endif /* !COMPLEX_FIELD_VALUES */
...
void
SchemeTMz::initScheme (FPValue wLength, FPValue step)
{
  waveLength = 0.0002;
  stepWaveLength = step;
...
//       }
  ...
  for (int i = 0; i < OmegaPE.getSize ().getX (); ++i)
  {
    for (int j = 0; j < OmegaPE.getSize ().getY (); ++j)
    {
      ...
      // {
#ifdef COMPLEX_FIELD_VALUES
        valOmega->setCurValue (FieldValue (sqrtf(2.0) * 2 * PhysicsConst::Pi * 30000000000, 0));
#else /* COMPLEX_FIELD_VALUES */
        valOmega->setCurValue (sqrtf(2.0) * 2 * PhysicsConst::Pi * 30000000000);
#endif /* !COMPLEX_FIELD_VALUES */
      }
