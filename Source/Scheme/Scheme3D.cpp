  ...
  for (int i = ExStart.getX (); i < ExEnd.getX (); ++i)
  {
    for (int j = ExStart.getY (); j < ExEnd.getY (); ++j)
    {
      for (int k = ExStart.getZ (); k < ExEnd.getZ (); ++k)
      {
        ...
        FPValue sigmaX = yeeLayout->getMaterial (posAbs, GridType::DX, SigmaY, GridType::SIGMAY);
  ...
  for (int i = HxStart.getX (); i < HxEnd.getX (); ++i)
  {
    for (int j = HxStart.getY (); j < HxEnd.getY (); ++j)
    {
      for (int k = HxStart.getZ (); k < HxEnd.getZ (); ++k)
      {
        ...
        FPValue sigmaY = yeeLayout->getMaterial (posAbs, GridType::HX, SigmaY, GridType::SIGMAY);
...
//         }
        ...
        //printf ("eps %d %d %d : %f %f\n", i, j, k, eps->getCurValue ().real (), eps->getCurValue ().imag ());
